...
/**
 * The site header — ONE definition, used by layouts/default.vue and error.vue.
 *
 * It lived in the layout, and error.vue kept a hand-copied version of the markup plus 6 of its 8 CSS
 * rules. The copy then rotted in exactly the way copies do: by 2026-07-20 the error page had silently
 * lost `position: sticky`, the `.nav-hidden`/`.nav-clear` scroll behaviour, the `@media (max-width:
 * 640px)` rule that hides the nav links on phones, and the skip link entirely. Nobody edited the copy
 * to break it — the original simply moved on without it. Three defects, one root cause, and the only
 * fix that holds is deleting the second copy.
 *
 * The skip link ships here too, so no consumer can forget it. Its target (`#main`) lives in the
 * consumer, which is the one coupling worth keeping: every page needs a <main id="main">.
 */
...
/* off-screen until keyboard focus lands on it; then a small card above the nav.
   Not inside <header>, deliberately: .nav-hidden takes a transform, and a transformed ancestor
   becomes the containing block for position:fixed descendants — the skip link would ride the
