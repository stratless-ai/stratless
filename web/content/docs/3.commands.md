...
4. **It quotes the build, then asks once.** It shows an honest estimate (moments, dollars, minutes, all on your own `claude`), then asks `Build your profile now? [y/N]`. The default is no. Say yes and it builds now; say no and nothing is spent, the free read stays, and the refresh keeps it current. The paid build never runs without that explicit yes on a real terminal.
...
Reads every session that is new since last time, scores it against the columns discovered from your own history, and rebuilds the profile. Run by hand in a terminal, it rebuilds now. Run by the after-session refresh, it rebuilds on a cadence: at most once a week by default, or once a day with `--daily`. `update` itself always rebuilds immediately, whatever the cadence is set to; the flags only change how often the background refresh acts on its own.
...
$ stratless status
  ...
  stratless status
    ...
    spend
      total          $13.27  ·  2.1M tokens  ·  1,412 calls  on your own claude
      by stage       discovering $4.80 · scoring $7.20 · writing $1.27
```
...
Add `--check` and it makes exactly one announced request to the npm registry to tell you whether a newer version exists. That is the only way stratless ever touches the network beyond your own `claude`: you ask, it checks, on screen.
...
The measured portrait of you, across every conversation, not one project. It is pure arithmetic on your own history: no model call, no tokens, instant and free. `course corrections` is how often you cut in to redirect your assistant, per hundred messages you send; `profile captures` is how much of you the current build's columns actually cover. Everything here is read locally and spends nothing.
