...
$ stratless tune
  ...
  Stage 3 · 6 skills + 3 styles

  SKILLS — habits at their moments
    ⚙ ask-assistant-to-verify-remote-state  (198×)  new
    ⚙ give-minimal-continuation-signal  (207× · slip 16×)  new
    ⚙ request-a-plan-before-implementing  (270× · slip 28×)  new
    ⚙ ask-how-to-set-up-integration  (288×)  new
    ⚙ open-strategic-discussion-topic  (298×)  new
    ⚙ propose-then-ask-for-plan  (232×)  new

  STYLE — always on
    ≈ check-in-on-session-progress  (230×)  new
    ≈ give-terse-followup-directive  (329×)  new
    ≈ riff-on-product-direction  (300×)  new

  5 rows stay rows — the map carries them, the tune does not.

  writes to ~/.claude/skills and ~/.stratless/tune/claude-code · nothing else touched
  every receipt is your own count · remove any time: stratless stop

  Install? [y/N]
```

Turns your logs into skills. `tune` reads the base map your history already built, derives the
skills it supports — real habits your assistant gains, each one a moment you kept having to ask
for — and prints the whole prescription before anything happens: every skill with the count that
earned it, every file path it would write, and one question. The default is no. Nothing lands
without a typed yes on a real terminal, and nothing outside the named paths is ever touched.

The **stage** is how much tune your record supports, and it is prescribed, never chosen: a young
pair gets a simple tune-up (one real habit, felt immediately); a long, worked-in pair earns the
full set — habits covering the task loop end to end. A `slip` receipt marks the standards your
assistant kept missing even though you asked: measured failures, fitted with countermeasures.

Skills your own setup already covers are never duplicated — `tune` reads what is installed
first, skips anything an existing skill of yours already does, and says so. Your skills are
yours; it never edits or replaces them. Run `tune` again after an `update` and it re-derives
from the fresh map, showing exactly what changed — new, updated, unchanged — before asking
again. Every skill also carries its retirement condition in the file itself: when your pair
absorbs a standard, the skill is built to come back out. `stratless stop` removes the whole
tune along with everything else.

Today `tune` derives for the Claude Code pair; the Codex pair's delivery follows once its
tool-side behaviours are measured against the real thing, the same way every leg ships.
...
It reads every available `HUMAN.<assistant>.md` at the moment it is asked, so a rebuild reaches every connected client at once with nothing to re-install. Each profile is labelled with the assistant relationship that produced it, and the response says explicitly that none of those observations were measured with the MCP client. There is **one tool and it only reads** — nothing here can edit a profile, propose a change to it, or write anything at all, because each profile is derived from its pair's own history by arithmetic and there is nothing for an assistant to edit. If no profile has been built yet, it says so plainly and tells the assistant to treat you as unknown rather than guess.
