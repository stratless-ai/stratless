...
Four things, all on your machine. `stratless stop` removes the hooks and unloads the profiles; it deliberately leaves your profile files, archive, local engine, and Claude Code retention setting in place.
...
Each `HUMAN.<assistant>.md` is a plain text file on your machine. It is yours: read it, edit it, load it elsewhere with its source relationship made clear, or delete it. The files are never uploaded, pooled, or shared. `stratless stop` unloads every one in a single command without deleting them.
