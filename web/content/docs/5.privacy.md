...
**2. A one-time model download, at `init`, with your consent.** The pattern-finding runs on a small open-weights model (~34MB) that lives on your machine. It has to arrive once. `init` tells you before it happens and asks; nothing downloads in the background, and the after-session refresh never fetches anything. **After it lands, that step is permanently offline** — no network, no provider, no bill, however many times you rebuild.
