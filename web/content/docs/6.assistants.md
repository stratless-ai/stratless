...
**Loading** is solved for everything at once. `stratless mcp` serves the available pair profiles over the Model Context Protocol, each labelled with the assistant relationship that produced it, so any assistant that speaks MCP can read them without stratless knowing anything about that tool's files. It never presents one pair's observations as if they were measured with the MCP client.
...
Run `stratless mcp` and point an assistant at it, and it can read every available pair profile, labelled by where each was measured. No reader, no adapter, nothing per-tool. One config line, verified working today in:
...
Cline and Copilot keep local session logs detailed enough to carry both sides of a conversation, which is what a pair profile needs. These come next: another `HUMAN.<assistant>.md`, with a per-tool reader.
