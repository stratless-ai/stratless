...
`profile` prints every current pair profile, free and instant. Each is the same `HUMAN.<assistant>.md` that assistant loads; `profile` just shows the files to you, and never spends. Building and loading them is `update`'s job. See [Commands](/docs/commands).
