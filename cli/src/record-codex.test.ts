...
import { driftCheck, readSessions, readTurns, roots, turnsOfFile, archiveSlice } from './record-codex.js';
...
test('the reader streams: a rollout is never held in memory whole', () => {
  ...
  const here = dirname(fileURLToPath(import.meta.url)); // dist/ at run time
  const src = readFileSync(pathJoin(here, '..', 'src', 'record-codex.ts'), 'utf8');
