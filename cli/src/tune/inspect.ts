 ...
 *
 * A real shop doesn't sell you a second intake. Before the door prints its report, every
 * proposal is compared against what the machine already runs: the person's installed skills and
 * always-loaded blocks (theirs — NEVER touched, and capable of covering a proposal) and the
 * harness's native features (factory parts — a proposal drives them, never rebuilds them).
 * Comparison runs on the DESCRIPTION surface — the text skills actually compete on at invocation
 * time — through the shipped embedding path, by arithmetic.
 *
 * Two kinds of installed skill, told apart by the mint mark ("Minted by stratless"):
 *   THEIRS — user-authored. A close-enough match covers the proposal: verdict `already-fitted`,
 *   named, nothing written — the report VALIDATES their pick. The person's setup is the authority.
 ...
 * (Solo V2, step 5 — the calibration survives the deriver it was built beside.)
 */
...
/**
 * Read every always-loaded block file (*.md) under the given dirs — the tune's second surface.
 * These never carried frontmatter, so the description is the first real paragraph: comments,
 * headings, and blank lines stripped. Same defensive posture as the skills reader.
 */
export function readInstalledBlocks(dirs: string[]): InstalledSkill[] {
  const out: InstalledSkill[] = [];
  for (const dir of dirs) {
    if (!existsSync(dir)) continue;
    let entries: string[];
...
/** A cover verdict on the description surface alone — what the sitting's proposals are judged
 *  on. `kind` is absent here; the caller knows what it asked about. */
...
/**
 * Inspect descriptions against the installed world. One embedding pass, nearest-cover wins,
 * floor-gated, deterministic ties by cover name. Minted-by-us installed skills are excluded
 * from coverage by construction — a tune replacing itself is an update, not a collision.
 */
export async function inspectDescriptions(
  items: { name: string; description: string }[],
  installed: InstalledSkill[],
  embed: Embedder,
): Promise<CoverVerdict[]> {
  const covers = [
    ...installed.filter((s) => !s.minted).map((s) => ({ name: s.name, text: s.description })),
    ...NATIVE_FEATURES.map((n) => ({ name: n.name, text: n.text })),
  ];
  if (items.length === 0) return [];
  if (covers.length === 0) return items.map((it) => ({ name: it.name, verdict: 'mint' as const }));

  const vectors = await embed([...items.map((it) => it.description), ...covers.map((c) => c.text)]);
  const itemVec = (i: number): Float32Array => vectors[i]!;
  const coverVec = (i: number): Float32Array => vectors[items.length + i]!;

  return items.map((it, ii) => {
    let best: { name: string; link: number } | undefined;
    covers.forEach((c, ci) => {
      const link = dot(itemVec(ii), coverVec(ci));
      if (link < INSPECT_FLOOR) return;
      if (!best || link > best.link || (link === best.link && c.name < best.name)) best = { name: c.name, link };
    });
    return best
      ? { name: it.name, verdict: 'already-fitted' as const, coveredBy: best.name, link: best.link }
      : { name: it.name, verdict: 'mint' as const };
  });
}
