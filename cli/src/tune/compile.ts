...
export function guidePrompt(evidence: EvidenceItem[], installed: InstalledNote[]): string {
  ...
  return [
    ...
    ...(mintedLines.length
      ...
      : []),
    'Propose at most five skills that convert what this person demonstrably values into something they no longer carry. Prefer skills that DO something — run a chain, perform a check, execute a sequence — over skills that merely remind or advise. Rules, absolute:',
    ...
    '- description: third person, what + when, the person\'s own phrases as trigger words — be a little pushy about when it applies, and always end the description with one thing it must NOT be used for',
    '- standard: one sentence; moves: two to five imperative lines the assistant executes; the LAST move is the self-test — the one check that proves the move actually happened',
    '- close the moves with where the skill ends: do the thing, then stop',
    '- evidence marked [patch] is a standard this person demonstrably holds but arrives at late — the strongest skill you can propose is the one that runs that standard for them',
    '- write in plain imperatives with a reason where one helps; never all-caps, never shouted absolutes',
