...
import { acquireLock, releaseLock, readLock, lockIsStale, processCommand, spawnDetached } from '../runner/worker.js';
import { summarizeTurns, appendRun, stageRates, etaMs, startRun, STOPWATCH_KEEP } from '../runner/stopwatch.js';
import { runClaude, parseJsonResult, CLEAN_ARGS, TOOLLESS_ARGS } from '../integrations/brains/claude-code.js';
import { readState, writeState, type RunRecord } from '../runner/state.js';
...
test('C2: kill -9 mid-write leaves a parseable store — old version or new, never torn', async () => {
  ...
  writeFileSync(
    writer,
    [
      `import { atomicWriteFileSync } from '${moduleUrl('../storage/atomic.js')}';`,
...
test('C4: simultaneous acquirers — exactly one wins', async () => {
  ...
  writeFileSync(
    contender,
    [
      `import { acquireLock } from '${moduleUrl('../runner/worker.js')}';`,
...
test('C4: a live pid that is NOT a stratless-ish process is PID reuse — stale, stolen', { skip: PS_UNAVAILABLE ? '`ps` is unavailable in this sandbox' : false }, async () => {
...
test('C5: a detached child survives its parent dying and finishes its work', async () => {
  ...
  writeFileSync(
    parent,
    [
      `import { spawnDetached } from '${moduleUrl('../runner/worker.js')}';`,
