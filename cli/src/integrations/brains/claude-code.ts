 ...
 * tool-bound (see `contracts.ts`): this is one way to ask a question, and it can read ANY assistant's
 * history. The most common machine there is reads Codex transcripts using this brain. So it is not
 * Claude Code's brain, it is the brain that arrives with Claude Code — `brains/registry.ts` chooses between
 * it and `codex.ts` by what is installed, and that choice belongs to the person, never to an
 * adapter.
 */
import { execFileSync } from 'node:child_process';
import { existsSync } from 'node:fs';
import { recordUsage, type CallCost } from '../../runner/usage.js';
