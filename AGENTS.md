...
- ✅ **One sample skill, one source — same law.** `web/content/samples/skill.md` is one real derived artifact, compiled by `cli/src/tune/compile.ts` from a live record and checked in verbatim (byte-identical to the compiler's output, never edited by hand). Every surface that shows the tune — the hero terminal's `stratless tune` beat, the README hero image, the npm README's excerpt, any docs sample — quotes lines from this file. Same discipline: update every quoting surface together or not at all.
